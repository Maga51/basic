(function(){function r(e,n,t){function o(i,f){if(!n[i]){if(!e[i]){var c="function"==typeof require&&require;if(!f&&c)return c(i,!0);if(u)return u(i,!0);var a=new Error("Cannot find module '"+i+"'");throw a.code="MODULE_NOT_FOUND",a}var p=n[i]={exports:{}};e[i][0].call(p.exports,function(r){var n=e[i][1][r];return o(n||r)},p,p.exports,r,e,n,t)}return n[i].exports}for(var u="function"==typeof require&&require,i=0;i<t.length;i++)o(t[i]);return o}return r})()({1:[function(require,module,exports){
    ...
    */
    ...
    document.addEventListener('DOMContentLoaded', () => {
        ...
        const qrLinkBtn = document.getElementById('qrcodeLink');
        const qrCodeCont = document.getElementById('qrcode-container');
        qrCodeCont.style.display = 'none';
        qrLinkBtn.addEventListener('click', () => {
            // Przełączanie widoczności
            if (qrCodeCont.style.display === 'none') {
                qrCodeCont.style.display = 'block';
            }
            else {
                qrCodeCont.style.display = 'none';
            }
        });
        // qrCodeDiv to div, w którym lib QRCode wstawi canvas z obrazkiem
        const qrCodeDiv = document.getElementById('qr-code');
        function qRCodeFunction() {
            const text = qrTextInput === null || qrTextInput === void 0 ? void 0 : qrTextInput.value;
            // Czyścimy poprzedni kod QR
            qrCodeDiv.innerHTML = '';
            // Generujemy nowy kod QR
            if (text.length > 10) {
                qrcode_1.default.toCanvas(text, { width: 200 }, (error, canvas) => {
                    if (error) {
                        console.error('Error generating QR code:', error);
                    }
                    else {
                        qrCodeDiv.appendChild(canvas); // Dodajemy kod QR do div-a
                    }
                });
            }
            else {
        ...
        generateBtn.addEventListener('click', qRCodeFunction);
        resetBtn.addEventListener('click', resetQRCode);
        qrTextInput.addEventListener('input', () => {
            const textLength = qrTextInput.value.trim().length;
            console.log('input', qrTextInput.value.trim());
            const check = textLength < 10;
            generateBtn.disabled = check;
        });
        const checkDivHeight = () => {
            if (qrCodeDiv.offsetHeight > 0) {
                resetBtn.style.display = 'block'; // Pokaż przycisk
            }
            else {
                resetBtn.style.display = 'none'; // Ukryj przycisk
            }
        };
    });
    ...
    if (layer1)
        layer1.style.transform = `translateX(-100px)`;
    if (layer2)
        layer2.style.transform = `translateX(-100px)`;
    if (layer3)
        layer3.style.transform = `translateX(-100px)`;
    document.addEventListener('mousemove', (event) => {
        ...
        if (layer1Element) {
            layer1Element.style.transform = `translateX(${offset1}px)`;
        }
        const layer2Element = document.getElementById('layer2');
        if (layer2Element) {
            layer2Element.style.transform = `translateX(${offset2}px)`;
        }
        const layer3Element = document.getElementById('layer3');
        if (layer3Element) {
            layer3Element.style.transform = `translateX(${offset3}px)`;
        }
    });
